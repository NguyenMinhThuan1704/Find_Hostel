...
export const getCodes = (arrMinMax, prices) => {
    ...
    return pricesWithMinMax.filter((item) => item.min === arrMinMax[0] && item.max === arrMinMax[1]);
};

export const getCodesArea = (arrMinMax, areas) => {
    const areasWithMinMax = getCodeArea(areas);
    return areasWithMinMax.filter((item) => item.min === arrMinMax[0] && item.max === arrMinMax[1]);
};
